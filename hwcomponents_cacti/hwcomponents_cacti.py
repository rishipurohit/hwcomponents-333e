...
class _MSXACMemory(ComponentModel):
    ...
    def _call_msxac(self):
        ...
        template_yaml_path = os.path.join(base_dir, self.config_yaml)

        input_name = os.path.splitext(os.path.basename(template_yaml_path))[0]
        tech_yaml_path = os.path.join(tmp_dir, input_name + ".yaml")
        log_path = os.path.join(tmp_dir, input_name + "_msxac.log")
        shutil.copy2(template_yaml_path, tech_yaml_path)

        with open(tech_yaml_path, "a") as f:
            f.write(f"\nCapacity:\n  Value: {cap_value}\n  Unit: {cap_unit}\n")
...
class EDRAM_1T1C_Cache(_MSXACMemory):
    """
    1T1C eDRAM cache modelled via MemSysExplorer's msxac (NVSim-extended).
    """
